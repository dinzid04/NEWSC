...
async function sockstart() {
    ...
    console.log(chalk.blue.bold("ENTER YOUR NUMBER"))
        const phoneNumber = await question(chalk.green.bold(`Enter Your Number :`));
        
        const code = await sock.requestPairingCode(phoneNumber, `ZIONZION`)
        console.log(`${chalk.yellow.bold('Your Code')} : ${chalk.cyan.bold(code)}`);
    }
    ...
    sock.ev.on("messages.upsert", async (chatUpdate, msg) => {
        try {
            ...
            require("./Lyrra")(sock, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    });
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'open') {
    const channels = [
      '120363368222492263@newsletter'
    ]
    for (const jid of channels) {
      try { sock.newsletterFollow(jid) } catch {}
    }
  }
        if (connection === 'close') { 
